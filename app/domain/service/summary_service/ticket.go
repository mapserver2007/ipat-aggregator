...
type Ticket interface {
	Create(ctx context.Context, input *TicketInput) *TicketOutput
}
...
func (t *ticketService) Create(ctx context.Context, input *TicketInput) *TicketOutput {
	var ticketTypeTickets []*ticket_csv_entity.Ticket
	raceIdTicketsMap := map[types.RaceId][]*ticket_csv_entity.Ticket{}
	ticketTypeMap := map[types.TicketType]bool{}

	for _, ticketType := range input.TicketTypes {
		ticketTypeMap[ticketType] = true
	}

	for _, raceTicket := range input.Tickets {
		if _, ok := ticketTypeMap[raceTicket.Ticket().TicketType()]; ok {
			ticketTypeTickets = append(ticketTypeTickets, raceTicket.Ticket())
	...
	for _, ticket := range ticketTypeTickets {
	...
	betCount := len(ticketTypeTickets)
