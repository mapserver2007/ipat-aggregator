...
type DistanceCategory interface {
	Create(ctx context.Context, input *DistanceCategoryInput) *DistanceCategoryOutput
}

type DistanceCategoryInput struct {
	Tickets            []*ticket_csv_entity.RaceTicket
	Races              []*data_cache_entity.Race
	DistanceCategories []types.DistanceCategory
}

type DistanceCategoryOutput struct {
...
type distanceCategoryService struct{}

func NewDistanceCategory() DistanceCategory {
	return &distanceCategoryService{}
}

func (d *distanceCategoryService) Create(ctx context.Context, input *DistanceCategoryInput) *DistanceCategoryOutput {
	var distanceCategoryTickets []*ticket_csv_entity.Ticket
	raceIdTicketsMap := map[types.RaceId][]*ticket_csv_entity.Ticket{}
	raceMap := map[types.RaceId]types.DistanceCategory{}
	for _, race := range input.Races {
		raceMap[race.RaceId()] = types.NewDistanceCategory(race.Distance(), race.CourseCategory())
	}

	for _, raceTicket := range input.Tickets {
		distanceCategory, ok := raceMap[raceTicket.RaceId()]
		if ok {
			if d.containsInSlice(input.DistanceCategories, distanceCategory) {
				distanceCategoryTickets = append(distanceCategoryTickets, raceTicket.Ticket())
	...
	for _, ticket := range distanceCategoryTickets {
...
func (d *distanceCategoryService) containsInSlice(slice []types.DistanceCategory, distanceCategory types.DistanceCategory) bool {
	for _, c := range slice {
		if c == distanceCategory {
			return true
		}
	}
	return false
